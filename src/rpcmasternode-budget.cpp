...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a ZixCash initiative\n"
            "  vote-alias         - Vote on a ZixCash initiative\n"
            "  vote               - Vote on a ZixCash initiative/budget\n"
...
Value preparebudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"zixcash-address\" monthy-payment\n"
            ...
            "5. \"zixcash-address\":   (string, required) ZIX address to send payments to\n"
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ZixCash address");

    // Parse ZixCash address
...
Value submitbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 7)
        throw runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"zixcash-address\" monthy-payment \"fee-tx\"\n"
